...
appearance of the early GWAS (Figure \@ref(fig:article-counts)). Inverse variance weighted MR [@burgess_mendelian_2013], MR-Egger [@bowden_mendelian_2015], weighted median estimation [@bowden_consistent_2016], Bayesian weighted MR [@zhao_bayesian_2019], robust adjusted profile score [@zhao_statistical_2020], MRMix [@qi_mendelian_2019], CAUSE [@morrison_mendelian_2020], and MRAID [@yuan_likelihood_2021] are all recent methods developments. These methods differ in their approaches to two challenges: 
...
variable. The LD clumping procedure then selects SNPs that are not in high LD 
with the top-ranking SNPs. 
...
happen, for instance, by choosing a noncausal SNP that is in high LD with a 
causal SNP. 

@verbanck_detection_2018
...
GWAS summary statistics [@burgess2015using]. These methods are especially pertinent now that there is 
an abundance of freely available GWAS summary data [@buniello2019nhgri]. GWAS summary statistics 
typically consist of an identifier and a p-value for every SNP in the data set, along with the sample 
size [@buniello2019nhgri].
...
The outcome variable can be binary, a count, or a continuous variable. For brevity and simplicity, 
we focus on continuous outcomes below.
...
the strength of association between the SNP and exposure. 
...
@burgess_sensitivity_2017 divided the suite of methods for sensitivity analysis 
into two categories: 1) those that support the validity of causal inferences in a MR 
study and 2) those that question that validity. Because not all confounding 
...
methods with multiple SNP instruments [@hormozdiari_leveraging_2018;@watanabe_global_2019]. 

Choosing a subset of the available SNPs to serve as instruments is one consideration that arises 
in MR studies. The solution to this task differs among published methods. Early methods chose a 
set of independent SNPs. However, this approach can be problematic when there are multiple causal 
...
exposure $n$ vector $X$ on the $n$ by $S$ matrix of SNP instruments. Taking the fitted exposure values 
from this regression, the second regression involves regressing the outcome variable $Y$ on 
the fitted values. 
...
where $\epsilon$ is assumed to be a vector of iid normal random variables with mean zero. 
...
further research into methods to model and to account for horizontal pleiotropy [@verbanck_detection_2018].

Early MR analyses avoided confounding from horizontal pleiotropy by discarding 
instrumental SNPs that might be associated with the outcome [@verbanck_detection_2018;@zhu_causal_2018;@zeng_causal_2019-1;@zeng_causal_2019-2].
More recent methods have attempted to model horizontal pleiotropy [@morrison_mendelian_2020;@yuan_testing_2020;@yuan_likelihood_2021]. CAUSE [@morrison_mendelian_2020] mdoels two varieties of horizontal pleiotropy: 
